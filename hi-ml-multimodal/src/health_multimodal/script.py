...
from model_import import get_biovil_t_image_encoder, save_checkpoint
from torch.optim.lr_scheduler import LambdaLR
import argparse
import pandas as pd
from MDataset import MimicDataset
# %%
...
parser.add_argument("--cuda", "-cuda", type=int, required=True, help="cuda number be specified")
parser.add_argument("--findings", "-findings", type=str, required=True, help="findings be specified")
...
model.classifier= MultiTaskModel(input_dim = model.feature_size ,classifier_hidden_dim=None, num_classes=6, num_tasks=1)
...
train_dataset= MimicDataset(csv_path ="/home/irail/hi-ml-multimodal/data_prep/mimic_interval_org_prep.csv", findings=f"{args.findings}", is_test= False)
test_dataset = MimicDataset(csv_path ="/home/irail/hi-ml-multimodal/data_prep/mimic_interval_org_prep.csv", findings=f"{args.findings}", is_test= True)
